...
try:
    ...
    print(f"Catalog might already exist: {e}")
...
try:
    ...
    print(f"Account table might already exist: {e}")
...
try:
    ...
    print(f"Product table might already exist: {e}")

# Process topics using SQL approach
print("Using SQL approach for data processing...")
kafka_source_account_ddl = """
    CREATE TEMPORARY TABLE kafka_account_source (
        ...
        'properties.bootstrap.servers' = 'kafka-standalone:19092',
...
print(f"Account job started: {account_job.get_job_client().get_job_id()}")
...
print(f"Product job started: {product_job.get_job_client().get_job_id()}")

print("Jobs submitted successfully. Processing data...")
